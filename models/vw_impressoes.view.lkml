view: vw_impressoes {
  ...
  FROM
   (select dimension_date , dimension_ad_unit_id,sum(a.column_total_line_item_level_impressions ) total_impressoes from `etusbg.dfp_publishers.pagina_anuncio_bloco` a
   group by dimension_date , dimension_ad_unit_id) x
  LEFT JOIN
    `etusbg.dfp_publishers.anuncio_bloco` ab
 ON
  ...
  FROM
    `etusbg.dfp_publishers.pagina_anuncio_bloco` p
  WHERE
